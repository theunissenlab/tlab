function ldata = LyonPassiveEar_new_mod(x,sr,df,low_freq,high_freq,earQ,stepfactor,differ,agcf,taufactor)
...
if df < 1; df = 1; end
if nargin < 4; low_freq=[];end
if nargin < 5; high_freq=[];end
if nargin < 6; earQ = 8; end
if nargin < 7; stepfactor = earQ/32; end
if nargin < 8; differ=1; end
if nargin < 9; agcf=1; end
if nargin < 10; taufactor=3; end

[earFilters, CenterFreqs, gains,list_max_vals]= DesignLyonFilters_new_mod(sr, earQ, stepfactor,low_freq,high_freq);


nSamples = length(x);
nOutputSamples = floor(nSamples/df);
[nChannels filterWidth] = size(earFilters);
fprintf('The number of neurons is: %d \n',nChannels);
...
ldata = struct;
ldata.spec = y;
ldata.freqs = CenterFreqs;
ldata.gains = gains;
ldata.nChannels = nChannels;
ldata.list_max_vals = list_max_vals;
