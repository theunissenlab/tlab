...
total_duration = 6000;                  % Total duration in ms- will give variable after stimulus time
...
after_stim = total_duration - before_stim - response.stim_duration*1000;  % In ms
v_axis(1) = -before_stim/1000.0;    % The axis is in seconds
...
ntimebins = fix(before_stim + response.stim_duration*1000 + after_stim); % Number of time bins in ms
...
for it=1:ntrials
    ...
    if (it <= nplot)
        ...
        % Plot a thin gray like indicating background
        if isfield(trial, 'pre_time') 
            plot([-trial.pre_time response.stim_duration+trial.post_time], [0 0], 'Color', [0.5 0.5 0.5], 'Linewidth', 1);
        end
        
        axis([v_axis(1) v_axis(2) 0 1]);
        axis off;
        hold off;
    end
end
...
maxpsth = max(smpsth)*1000;
if maxpsth == 0
    maxpsth =1;
end
axis([v_axis(1) v_axis(2) 0 maxpsth]);
