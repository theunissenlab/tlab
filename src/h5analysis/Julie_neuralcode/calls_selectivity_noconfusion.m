function [dsel, AvgDprime, StdDprime, Comptype, AvgRcallcat,StdRcallcat]=calls_selectivity_noconfusion(MatfilePath)
confusioncallcategory=1; %set this value to 1 if you want confusion matrix calculated based on call categories and not per stim 
Cal = struct();
...
winSize = [1 3 5 10 30 50 80 120 200];    % Window size for static Info calculation and for ideal observer
...
%% Calculations!!
nfiles=0; %to desenable confusion matrix calculation because it's aking too long!!!
if nfiles
    % With VR_distanceB
    for is=1:ns
        ...
        if confusioncallcategory==1
            ...
            [pc, mi_conf, zdT, pzdT, mi_zdT, mi_zdT_nc, zdS, pzdS, mi_zdS, mi_zdS_nc, confusion_matrix] = info_distanceB(nfiles, SpikeTrains , Sections_len, @VR_distanceB, winSize(is));
        end
...
if ismac()
        ...
        if strcmp(strtok(username), 'frederictheunissen')
            ...
            elseif strncmp('/auto/fdata/julie',stim_name, 17)
                calfilename = fullfile('/Users','frederictheunissen','Documents','Data','Julie','matfile',Res.subject,['Cal' Res.Site '_nc.mat']);
            end
        elseif strcmp(strtok(username), 'elie')
            calfilename = fullfile('/Users','elie','Documents','MATLAB','data','matfile',Res.subject,['Cal' Res.Site '_nc.mat']);
        end
else
    calfilename=fullfile('/auto','k6','julie','calmatfile',Res.subject,['Cal' Res.Site '_nc.mat']);
end
