function [Section_Trials,psth, stim_mean_rate, spike_std_rate, spike_zscore, pvalue, tvalue, pvalue_rand, spike_rate_stim] = spikeTimes_psth_cal(begInd, endInd, samprate,response,spike_rate_bg, Win)
...
Windur=Win*1000; %here the output psth is completed with zeros if needed to match a size of Windur ms
%Padding of psth with zeros desactivated because it biased results see
%l.40.
...
% elongate the psth vector if the section was shorter than 200ms
if ntimebins<Windur
    %psth=[psth zeros(1,Windur-ntimebins)]; Not doing it anymore because
    %it's biasing the discrimination of short calls
