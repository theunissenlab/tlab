function [nsections, pTHRESH, Section_pvalue, NSignif_Sections, NSignif_Sections_rand, NSignif_Sections_FakeTrial, pTHRESH_Deg, NSignif_Sections_Deg, NSignif_Sections_Deg_rand, NSignif_Sections_Deg_FakeTrial]=Sort_Multiunit_h5_file(h5Path,pl)
%% This function cut tdt stim to isolate vocalizations as much as possible,...
...and create 200ms windows of analysis. Then calculate spike rate...
   ...and sort h5 depending on their level of significant responses to stim...
    ...vs background. Then add some fields in h5 file to indicate # of significant cuts.
...
if classId ~= 0
    ...
    %% Now run a second cutting system based on the intensity of the...
    ...signal to get nicer cuts and enlarged the saved window around...
        ...the vocalization
    ...
    Section_pvalue=zeros(nfiles*coeff,3);
    Section_pvalue_rand=cell(nfiles*coeff,1);
    Section_cat=cell(nfiles*coeff,1);
    Section_pvalue_FakeTrial=cell(nfiles,1);
    ISI=cell(nfiles,2); %Inter-spike intervals
    histDur2=[];
    nsections = 0;
    Ag = zeros(1,10);
    Ag(1)=1;
    ...
    for isound = 1:nfiles
        ...
        %% Isolate spikes that relate to before the 1rst vocalization and...
        ...calculate avg background spike rate
        ...
        Spike_rate_isound = zeros(ntrials,coeff);
        Spike_rate_isound(:,1)=spike_rate_bg;
        ...
        for i=1:length(on)
            ...
            elseif (laston~=0) && (on(i) == 0) || (laston~=0) && (i==length(on))
                ...
                %% depending on section length enlarged or regularly cut...
                ...to obtain sections of 200ms each
                if durInd<Lsection && durInd>ceil(0.03*samprate)%section shorter than 200ms
                    ...
                    [Section_Trials,psth,stim_mean_rate, spike_std_rate, spike_zscore, pvalue, tvalue, pvalue_rand, spike_rate_stim] = spikeTimes_psth_cal(begInd, endInd, samprate,response,spike_rate_bg);
                    ...
                    Section_pvalue(nsections,1)=str2double(response.number);
                    Section_pvalue(nsections,2)=ord;
                    Section_pvalue(nsections,3)=pvalue;
                    Section_pvalue_rand{nsections}=pvalue_rand;
                    Spike_rate_isound(:,ord+1) = spike_rate_stim;
                ...
                elseif durInd>Lsection %section longer than 200ms
                    ...
                    for ns=1:nsec
                        ...
                        % Isolate spikes that relate to the section and...
                        ...calculate average (psth) for this section
                        [Section_Trials,psth,stim_mean_rate, spike_std_rate, spike_zscore, pvalue, tvalue,pvalue_rand, spike_rate_stim] = spikeTimes_psth_cal(local_begInd, local_endInd, samprate,response,spike_rate_bg);
                        MeanRate{nsections}=stim_mean_rate;
                        StdRate{nsections}=spike_std_rate;
                        Section_zscore{nsections}=spike_zscore;
                        Section_tvalue{nsections}=tvalue;
        ...
        Spike_rate_isound_rand = zeros(ntrials,ord+1);
        for it=1:ntrials
            Spike_rate_isound_rand(it,:) = Spike_rate_isound(randperm(ord+1));
        end
        
        %% Calculate random pvalue for spike rate
        Background_rate_TrialFake = Spike_rate_isound_rand(:,1);
        Stim_rate_TrialFake = Spike_rate_isound_rand(:,(2:(ord+1)));
        Pvalue_FakeTrial_isound = zeros(ord,1);
        for iord = 1:ord
            drate_Fake = Stim_rate_TrialFake(:,iord) - Background_rate_TrialFake;
            mdrate_Fake = mean(drate_Fake);
            if (mdrate_Fake == 0 )
                spike_zscore_Fake = 0;
            else
                spike_zscore_Fake = mdrate_Fake./std(drate_Fake);
            end
            tvalue_Fake = spike_zscore_Fake*sqrt(ntrials);
        
            % Two tailed t-test pvalue
            Pvalue_FakeTrial_isound(iord) = 2.0.*(1-tcdf(abs(tvalue_Fake),ntrials-1));
        end
        Section_pvalue_FakeTrial{isound}=Pvalue_FakeTrial_isound;
            
        %% Calculate interspike intervals (not really smartly done here,...
        ...see spike_auto_correlation_h5)
        %Trial_num=length(response.trials);
        %Spikes_num=zeros(Trial_num,1);
        %for iT=1:Trial_num
        %    Spikes=response.trials{iT};
        %    Spikes_num(iT)=length(Spikes.spikeTimes)-1;
        %end
        %isi=zeros(sum(Spikes_num),1);
        %idx_isi=1;
        %for iT=1:Trial_num
        %    Spikes=response.trials{iT};
        %    isi(idx_isi:(idx_isi+Spikes_num(iT)-1)) = abs(Spikes.spikeTimes(1:(end-1)) - Spikes.spikeTimes(2:end));
        %    idx_isi = idx_isi + Spikes_num(iT);
        %end
        %ISI{isound,1}=isi;
        %ISI{isound,2}=length(isi);
        
    end
    %hist(histDur2,60)
    
    %% histogram of Interspike interval for that unit 
    %ISI_num=sum(cell2mat(ISI(:,2)));
    %ISI_cumsum=cumsum(cell2mat(ISI(:,2)));
    %ISI_vec=zeros(ISI_num,1);
    %idx_ISI=1;
    %for isound=1:nfiles
    %    ISI_vec(idx_ISI:ISI_cumsum(isound))=ISI{isound,1};
    %    idx_ISI=ISI_cumsum(isound)+1;
    %end
    %hist(ISI_vec,1000)
    %mean(ISI_vec)
    %axis([0 0.005 0 12000])
    %sum(ISI_vec<0.001)/length(ISI_vec)
    
        
    
    %% Bonferrnni correction on pthreshold and keep h5 in folder only if...
    ...the unit respond significantly to one section
    MSection_pvalue=Section_pvalue(1:nsections,3);
    pTHRESH = pthreshold/nsections;
    Signif_Sections=MSection_pvalue'<pTHRESH;
    NSignif_Sections=sum(Signif_Sections);
    
    %% Bonferrnni correction on pthreshold calculated for shuffled stim/background
    MSection_pvalue_rand=cell2mat(Section_pvalue_rand(1:nsections));
    pTHRESH = pthreshold/nsections;
    Signif_Sections_rand=MSection_pvalue_rand'<pTHRESH;
    NSignif_Sections_rand=sum(Signif_Sections_rand); 
    
    %% Bonferrnni correction on pthreshold calculated for shuffled...
    ...stim/background per trial
    MSection_pvalue_FakeTrial=cell2mat(Section_pvalue_FakeTrial);
    pTHRESH = pthreshold/nsections;
    Signif_Sections_FakeTrial=MSection_pvalue_FakeTrial'<pTHRESH;
    NSignif_Sections_FakeTrial=sum(Signif_Sections_FakeTrial);
    
    
    %% Degressive Bonferroni correction
    Sorted_pvalues = sort(MSection_pvalue);
    ...
    %% Degressive Bonferroni correction for shuffled stim/background
    Sorted_pvalues_rand = sort(MSection_pvalue_rand);
    pTHRESH_Deg = repmat(pthreshold, 1, nsections) ./ sort((1:nsections),2,'descend');
    Signif_Sections_Deg_rand=Sorted_pvalues_rand'<pTHRESH_Deg;
    NSignif_Sections_Deg_rand=sum(Signif_Sections_Deg_rand);
    
    %% Degressive Bonferroni correction for shuffled stim/background per trial
    Sorted_pvalues_FakeTrial = sort(MSection_pvalue_FakeTrial);
    pTHRESH_Deg = repmat(pthreshold, 1, nsections) ./ sort((1:nsections),2,'descend');
    Signif_Sections_Deg_FakeTrial=Sorted_pvalues_FakeTrial'<pTHRESH_Deg;
    NSignif_Sections_Deg_FakeTrial=sum(Signif_Sections_Deg_FakeTrial);
    
    %%
    %pathPrefix = sprintf('/sections_data/%s', prot);
    pathPrefix = [sprintf('/extra_info/%s', prot) '/sections_data'];
    h5.set_ds(fid, pathPrefix, 'num_signif_sections_Bonferroni', NSignif_Sections);
    h5.set_ds(fid, pathPrefix, 'num_signif_sections_BonferroniDeg', NSignif_Sections_Deg);
    h5.set_ds(fid, pathPrefix, 'sections_pvalues', Section_pvalue(1:nsections,:));        
    h5.close(fid);    
    ...
    Section_pvalue=0;
    NSignif_Sections=0;
    NSignif_Sections_rand=0;
    NSignif_Sections_FakeTrial=0;
    pTHRESH_Deg=0;
    NSignif_Sections_Deg=0;
    NSignif_Sections_Deg_rand=0;
    NSignif_Sections_Deg_FakeTrial=0;
