function [filter_fit,params] = fit_filter(calc_dir,do_plot,verbose)
...
samprate = g.ampsamprate;
f_range = g.endFreq - g.initialFreq;
f_band = f_range/(g.NBAND - 1);
f = (0:g.NBAND-1) * f_band + g.initialFreq;
t0 = g.TimeLag + 1;
...
s = load(fullfile(calc_dir,'best_strf.mat'));
forward = s.strf(:,t0:end);
nt = size(forward,2);
t = 0:nt-1;
...
filter_fit.msbmf = cm_dwt;
filter_fit.msbms = cm_dwf.*1000.0;
...
filter_fit.f_X0 = (g.initialFreq + filter_fit.f_X0 * f_band)/1000.0;
