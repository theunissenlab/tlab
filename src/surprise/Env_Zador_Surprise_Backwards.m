...
stims = {};
stim_names = {};
rootSpecsDir = '/auto/fdata/pgill/Zador/Spectrograms';
dirout = dir(fullfile(rootSpecsDir,'class*'));

for jj = 1:length(dirout)
    allSpecs = dir(fullfile(rootSpecsDir,dirout(jj).name,'250','*.mat'));
    for kk =1:length(allSpecs)
        loaded = load(fullfile(rootSpecsDir,dirout(jj).name,'250',allSpecs(kk).name));
        stims{end+1} = loaded.outstim;
        stim_names{end+1} = [dirout(jj).name '_' allSpecs(kk).name];
    end
end
stim_familiarity = ones(1,length(stims));
stim_used = stim_familiarity;
%}
load /auto/fdata/pgill/Zador/EnvPlusStims.mat

savedir = '/auto/fdata/pgill/Zador/Surprise/in_progress_env';
fiatdir(savedir);

for f_band = 86:-1:1%:86
    %f_band = 22;
    try
