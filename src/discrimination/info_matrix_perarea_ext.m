function [ mi_tot, mi_tot2, mi_diag, mi_error, mi_diag_uni, mi_all_error_uni, mi_diag_uni_cat, mi_real_error_uni, mi_diag_uni_cat_maxInv, mi_diag_uni_cat_minInv]=info_matrix_perarea_ext(prob_matrix, cat,FigFlag)
...
if FigFlag==1
    Clim=[0 max(max(prob_matrix))];
    createconfusionmatrix(prob_matrix_uni_cat_maxInv, Clim)
    figure(5)
    title(sprintf('MI UniDiagCat_maxInv Matrix=%f/n', mi_diag_uni_cat_maxInv));
end
...
if FigFlag==1
    Clim=[0 max(max(prob_matrix))];
    createconfusionmatrix(prob_matrix_uni_cat_minInv, Clim)
    figure(6)
    title(sprintf('MI UniDiagCat_maxInv_minInv Matrix=%f/n', mi_diag_uni_cat_minInv));
end
