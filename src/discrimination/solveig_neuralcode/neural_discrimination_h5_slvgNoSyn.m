function data = neural_discrimination_h5_slvgNoSyn(h5Path, Birdname)
...
ConSyn = {'Con', 'Syn'}; % we keep the same format even if no Syn values
...
if nfiles
        ...
        for Dist=1:NDist
            ...
            %% for 2m and 'Con' calls, we calculate the mutual info for all window Sizes
                ...
                output = struct('StimType', ConSyn(1), 'Distance', PropDist(Dist), 'nfiles', nFiles, 'winsize', winSize, 'ChosenWinSize', winSize(index), 'stimrate', stimrate, 'stdstimrate', stdstimrate, ...
                ...
                %% for the rest of distances, we use the winSize that gave the best results at 2m.
            ...
            % Output structure. Important values for later analysis are: stimrate, avgzscore, mean_dprime, baseline_dprime, mi_conf_perBird.
            output = struct('StimType', ConSyn(1), 'Distance', PropDist(Dist), 'nfiles', nFiles, 'winsize', winSize, 'stimrate', stimrate, 'stdstimrate', stdstimrate, 'backrate', backrate, 'stdbackrate', stdbackrate, ...
