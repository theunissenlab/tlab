...
if nfiles
   % Calculate dprime within
    ...
    % Calculate the ideal oberserver VR metric using a Trial to Trial
    % comparison
    for is=1:ns
    ...
    % With VR_distanceB using a Trial to Template comparison
    for is=1:ns
        [pc, mi_conf, zdT, pzdT, mi_zdT, zdS, pzdS, mi_zdS] = info_distanceB(nfiles, spike_times, stim_len, @VR_distanceB, winSize(is));
        percorrectB(is) = pc;
...
outputs = struct('nfiles', nfiles, 'winsize', winSize, 'stimrate', stimrate, 'stdstimrate', stdstimrate, 'backrate', backrate, 'stdbackrate', stdbackrate, ...
    ...
    'percorrectB', percorrectB, 'mi_confusionB', mi_confusionB, 'zdistTB', zdistTB, 'pzdistTB', pzdistTB, 'mizdistTB', mizdistTB, ...
                                                                'zdistSB', zdistSB, 'pzdistSB', pzdistSB, 'mizdistSB', mizdistSB, ...                                                                
