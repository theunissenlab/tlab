function test_outputnl2()
    ...
    [nlinfoDists, nlparamsDists] = estimate_outputnl_from_dists(linResp, psth, numTrials);
        
    [nlinfoSpline, nlparamsSpline] = estimate_outputnl_from_spline(linResp, psth);
    ...
    respFromP = fnval(nlinfoDists.outputNL, linResp);

    
    %% compute responses from cubic spline approx
    respFromCs = fnval(nlinfoSpline.outputNL, linResp);
    ...
    [cBound, cP] = compute_coherence_full(respFromP, modelResponse, wholeResponse(1, :), wholeResponse(2, :), ...
                                              sampleRate, numTrials, infoFreqCutoff, infoWindowSize);
    pPerfRatio = cP.info / cBound.info
    
    [cBound, cCs] = compute_coherence_full(respFromCs, modelResponse, wholeResponse(1, :), wholeResponse(2, :), ...
                                              sampleRate, numTrials, infoFreqCutoff, infoWindowSize);
    csPerfRatio = cCs.info / cBound.info
    ...
    plot(nlinfoDists.x, nlinfoDists.px, 'k-'); axis tight;
    ...
    plot(nlinfoDists.x, nlinfoDists.pspikex, 'r-'); axis tight;
    plot(nlinfoDists.x, nlinfoDists.pnospikex, 'b-'); axis tight;
    plot(nlinfoDists.x, fnval(nlinfoDists.outputNL, nlinfoDists.x), 'm-'); axis tight;
    plot(nlinfoDists.x, fnval(nlinfoSpline.outputNL, nlinfoDists.x), 'g-'); axis tight;
