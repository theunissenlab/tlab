function varargout = RFresorter_single(varargin)
...
vv = cat(3,PF.rec(:).v_final);
vs = cat(3,PF.rec(:).plexon_spikes_final);
vt = 1:size(PF.rec,2);
validtrials=[];
for ii = vt
    if isempty(PF.rec(ii).v_final)
        good = 0;
    else
        good = 1;
    end
...
if exist([fname(1:s1(end)) 'sortparams.mat'])==2
    sortparams = load([fname(1:s1(end)) 'sortparams.mat']);
    
    sortparams.ens{chstart+1} = [];
    ens = sortparams.ens;
...
global heightsmean heightsstd widthsmean widthsstd ens tmpclusters fname tmpsortidx clusize tmpunsortedidx vs tridx tmps2 retry undohistory selected skip_button chstart
...
while flag == 0
    ...
    if partialflag == 0 && ch < chstart+2
    
        currsort = currsort + 1;
        
        if currsort > length(uvsorts)
            if goodflag==1
                fc = finalcluster_single;
                uiwait(fc)
            end
...
%                 end
    ...
    elseif partialflag == 1 && ch < chstart+2
...
global heightsmean heightsstd widthsmean widthsstd goodflag fname tmpsortidx tmpunsortedidx clusize vs tridx tmps2 retry undohistory clu selected ens chstart
...
global skip_button selected chstart
% skip_button = 1;
skip_function(handles)
