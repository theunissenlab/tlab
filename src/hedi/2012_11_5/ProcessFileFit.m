function res=ProcessFileFit(fname,a,thr);
...
hashname=sprintf('%s/output/%s.mat',pathstr,name);

if ~exist(hashname, 'file')
    pax=GetFitCall(hashname,spec,z.t,z.f,X0,slope,intercept,thr); % very long unless hashname exist
else
    load(hashname);
    pax=sxpa;
end;
wavname=sprintf('%s/output/pAv3.%s.wav',pathstr,name);
...
resname=sprintf('%s/output/res.pAv3.%s.mat',pathstr,name);
res.data=pax;
