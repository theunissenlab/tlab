...
% Lowpass filter for temporal enveloppe.
d=fdesign.lowpass('N,Fc',10,250,samprate/increment);
Hd = design(d);
...
x2pow = sum(x2);
x2 = smooth(x2, 2*increment-1, 'loess');
x2 = x2.*(x2pow/sum(x2));
% x2 = x2';
...
tError = sum((tActual.data-tMarginal.data).^2)./(length(input)/samprate);   % The error is normalized by duration of signal - if we want to compare different stims...
fError = sum((fActual.data-fMarginal.data).^2)./(length(input)/samprate);
