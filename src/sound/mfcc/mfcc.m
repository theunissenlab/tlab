function [ CC, FBE, FBEf, MAGU, MAGf, frames ] = mfcc( speech, fs, Tw, Ts, alpha, window, R, M, N, L )
...
%   Author: Kamil Wojcicki, September 2011
    ...
    [H, MAGf, c] = trifbank( M, K, R, fs, hz2mel, mel2hz ); % size of H is M x K 
    FBEf = c(2:end-1);

    % Filterbank application to unique part of the magnitude spectrum
    FBE = H * MAGU; % FBE( FBE<1.0 ) = 1.0; % apply mel floor
