...
max_wt = 50;        % Upper cuttoff for temporal frequencies in Hz
min_wt = 0;        % Lower cuttoff for temporal frequencies
no_it = 30;       % Number of iterations for the spectrographic inversion
f_width = 250;      % Width of the gaussian filters (std) for the spectrographic representation in Hz.  
        ...
        while (i <= no_of_ripples) 
            ...
            for j=1:size(w_x) 
                if(ran_power_val < cum_power_dist(j)); 
                    ...
                    % and within the omega_t*omega_x < i region.  omega_x
                    % is in cycles /kHz thus 1000. See Singh and Theunissen
                    % 2003
