...
double **filter;
double *average_rand;
int nband, nlen, nlen_avg, trial_spikes;
int nspikes=0;
...
FILE *f_debug2;
   ...
   c_spike = (float *)calloc(c_n*2, sizeof(float));
	...
	for ( it = nt/2-1; ++it < nt; ) 
	{
		c_spike[2*(it-nt/2)] = cross_spike[it]*0.5*(1-cos((it*2)*M_PI/nt));
		if ( it != nt/2 )
			c_spike[2*(c_n-it+nt/2)] = cross_spike[it]*0.5*(1-cos((it*2)*M_PI/nt));
	}
   ...
   for ( ib = -1; ++ib < nb; )
	{
		...
		for ( it = -1; ++it < nt; ) c_stim[2*(it+istart)] = filter[ib][it]*0.5*(1-cos((it*2)*M_PI/nt)); 
#ifdef DEBUG
      if ( ib == nb-1 )
