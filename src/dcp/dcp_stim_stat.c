...
char fname[64];
   ...
   fscanf(f_count,"%d %d %d %f %f %f %d %d %f %f", &nrec, &nband, &windowms, &f_low, &f_high, &f_step, &lin_flg, &sil_window, &f_width, &amp_samp);
	TWINDOW = (int)ceil((windowms*amp_samp)/1000.0);
	printf("Starting dcp_stim. Parameters:\n");
   printf("nrec=%d nband=%d windowms=%d f_low=%g f_high=%g f_step=%g f_width=%g lin_flg=%d sil_window=%d amp_samp=%g TWINDOW=%d\n",
          nrec, nband, windowms, f_low, f_high, f_step, f_width, lin_flg, sil_window, amp_samp, TWINDOW);
   fclose(f_count);
   ...
   /* Allocate memory for the cross-correlation */
	tot_corr = (double *)calloc(2*TWINDOW+1, sizeof(double));
	ncorr = nband;
	...
	for ( ib1 = -1; ++ib1 < ncorr; ) 
	{
		env_corr[ib1] = (double *)calloc(2*TWINDOW+1, sizeof(double));
		ns_env_corr[ib1] = (int *)calloc(2*TWINDOW+1, sizeof(int));
		for ( irec = -1; ++irec < nrec; ) 
		{
			JN_env_corr[irec][ib1] = (double *)calloc(2*TWINDOW+1, sizeof(double));
			JN_ns_env_corr[irec][ib1] = (int *)calloc(2*TWINDOW+1, sizeof(int));
		}
	}
		...
		for ( ib1 = -1; ++ib1 < nband; )
		{
			for ( ib2 = ib1-1; ++ib2 < nband; )
			{
				...
				for ( it1 = -1; ++ it1 < nlen; )
				{
					...
					for ( it2 = it1-TWINDOW-1; ++it2 <= it1+TWINDOW; )
					{  
						if ( it2 < 0 ) continue;
						else if ( it2 < nlen) 
                  {
   ...
   /* Write out results for JN files - these are the delete one values */
	for ( irec = -1; ++ irec < nrec; )
	{
		...
		for (xb = -1; ++xb < ncorr; )
		{
			...
			fwrite(JN_env_corr[irec][xb] , sizeof(double), 2*TWINDOW+1, f_stim);
		}
		fclose(f_stim);
	}
   ...
   f_stim = fopen("stim_stat.avg","w");
	...
	for (xb = -1; ++xb < ncorr; )
	{
		...
		fwrite(env_corr[xb] , sizeof(double), 2*TWINDOW+1, f_stim);
	}
	fclose(f_stim);
