...
int twindow=200;
   ...
   argc --;
	...
	while ( argv[0][0] == '-' )
	{
		...
		else if ( strncmp(argv[0], "-amp_samp", 6) == 0 )
		{
			argc --;
			argv ++;
   ...
   while (argc)
	{
		...
		stim_fband2(stim,&nband,&nlen,sil_window,&stim_env,&f_low,&f_high,&f_width,&f_step, amp_samp);
		sil_window_len = (int)ceil(sil_window*amp_samp/1000);  /* Length of sil window in amp_samp rate */
		...
		for ( ib = -1; ++ib < nband; )
		{
			for ( it = -1; ++ it < nlen+2*sil_window_len; )
			{
				if ( lin_flg == 0 )
				{
		...
		count_avg += (nlen+2*sil_window_len)*pin->n; 

      /* Write out spectrogram */
		fprintf(f_check,"%s %d\n", argv[0], nlen+2*sil_window_len);
		sprintf(fname,"stim_spect%d.dat",nf);
		f_stim = fopen(fname,"w");
		for ( ib = -1; ++ib < nband; )
			fwrite(stim_env[ib], sizeof(double), nlen+2*sil_window_len, f_stim);
		fclose(f_stim);

		free_dcp_memory(&pin);
		free_env(nlen+2*sil_window_len,nband,stim_env);
		argc --;
		argv ++;
		nf ++;
	}
   ...
   fprintf(f_count,"%d %d %d %f %f %f %d %d %f %f\n", nf, nband, twindow, f_low,f_high,f_step, lin_flg,sil_window_len, f_width, amp_samp);
   fclose(f_count);
