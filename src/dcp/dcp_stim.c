...
char fname[64];
		...
		for ( ib1 = -1; ++ib1 < nband; )
		{
			for ( ib2 = ib1-1; ++ib2 < nband; )
			{
				for ( it1 = -1; ++ it1 < nlen; )
				{
					...
					{  
						if ( it2 < 0 ) continue;
						else if ( it2 < nlen) stim2 = stim_env[ib2][it2]-stim_avg[ib2];
						else break;
   ...
   f_stim = fopen("stim.avg","w");
	if ( f_stim == NULL )
	{
		printf("Error: Could not write output file stim.avg\n");
		exit(1);
	}
	...
	for (xb = -1; ++xb < ncorr; )
	{
		for ( st = -1; ++st <= 2*TWINDOW; )
		{
			...
			fprintf(f_stim, "%g ", env_corr[xb][st]);
		}
		fprintf(f_stim, "\n");
	}
	fclose(f_stim);
	printf("dcp_stim is done\n");
