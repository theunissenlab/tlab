...
#define MAX_JN 10
   ...
   matchflg = 1;
	...
	while ( argc && argv[0][0] == '-' )
	{
		...
		else if ( strncmp(argv[0],"-noJN",5) == 0 ) 
		{
			matchflg = 2;
		}
   ...
   /* Check files and count max number of samples */
	f_ntrials = fopen("spike_psth.count", "r");
	if ( f_ntrials == NULL )
	{
...
nt=0;
ntrial_number=(int *)calloc(nrec+1,sizeof(int));
while (fscanf(f_ntrials, "%d", &ntrial_number[nt]) != EOF)
  {
       nt ++;
  }
    nt--;
current_max_value=ntrial_number[nt];
