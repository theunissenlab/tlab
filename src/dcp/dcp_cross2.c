			...
			for ( j = -1; ++j < ptin1->nspikes; )
			{
				...
				for ( k = -1; ++k < ptin1_1->nspikes; )
				{
					...
					if ( fabs(dt) < TMAX )
					{
						...
						{
							auto_stim_rand[ibin]++;
							JNauto_stim_rand[nt][ibin]++;
						}
						else if ( time_spike1 <= pin1->pre && time_spike2 <= pin2->pre )
						{
							auto_back_rand[ibin]++;
							JNauto_back_rand[nt][ibin]++;
						}
					}
				}
				for ( k = -1; ++k < ptin2_1->nspikes; )
				{
					...
					if ( fabs(dt) < TMAX )
					{
						...
						&& time_spike2 > pin1->pre && time_spike2 <= pin1->pre+stim_dur)
						{
							cross_stim_rand[ibin]++;
							JNcross_stim_rand[nt][ibin]++;
						}
						else if ( time_spike1 <= pin1->pre && time_spike2 <= pin2->pre )
						{
							cross_back_rand[ibin]++;
							JNcross_back_rand[nt][ibin]++;
						}
					}
				}
			}

			/* The cross correlation of 2 */
			for ( j = -1; ++j < ptin2->nspikes; )
			{
				...
				for ( k = -1; ++k < ptin2_1->nspikes; )
				{
					...
					if ( fabs(dt) < TMAX )
					{
						...
						&& time_spike2 > pin1->pre && time_spike2 <= pin1->pre+stim_dur)
						{
							auto2_stim_rand[ibin]++;
							JNauto2_stim_rand[nt][ibin]++;
						}
						else if ( time_spike1 <= pin1->pre && time_spike2 <= pin2->pre )
						{
							auto2_back_rand[ibin]++;
							JNauto2_back_rand[nt][ibin]++;
						}
					}
				}
			}
