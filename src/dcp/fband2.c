...
/* ----------------------------- stim_fband2() ------------------------------- */
stim_fband2(pstim, nband, len, twindow, pow_level, f_low, f_high, f_width, f_step, amp_samp)
DCP_STIM *pstim;
int *nband;
int *len;
...
int istart, nwindow;
int i_low, i_high;
int twindow_len;    /* length of window in samp rate */
double n_step;
double *pr;
float *c_song, *c_temp;
double f2, fres, df;
...
/* #define DEBUG */
	...
	*len = (int)ceil(nframes*tstep);   /* Length of stim at samp rate */
	printf("Length of stimulus at samp rate %f is %d\n", amp_samp, *len);
	twindow_len = (int)ceil(twindow*amp_samp/1000);

	if ( tstep > 1.0 ) 
	{
		 printf("Error: Amplitude sampling rate is too low\n");
		 exit(1);
	}
	*pow_level = (double **)calloc(nbands+1, sizeof(double *) );
	for ( nb = -1; ++nb <= nbands; ) (*pow_level)[nb] = (double *)calloc(*len+2*twindow_len, sizeof(double));

	/* Allocate memory for complex array */
	nwindow = (int)ceil(twindow*pstim->samprate/1000.0);
	...
	*f_step = (*f_high-*f_low)/nbands;
	...
	for ( nb = -1; ++nb < nbands; )
	{
      ...
      wt_max = *f_width*M_PI*sqrt(2);               /* This is a good upper bound for maximum mod freq. */
		
		/* if ( 2*wt_max > amp_samp ) */
		if ( 0 )
		{
 /* I need to implement a different type of filter - some type of MA filter. Also need to figure out whether I need to filter in the frequency domain.. */
			/* This anti aliasing filter causes some problems */
			four1(c_temp-1, (unsigned long)c_n, (int)1);
			fresampmean = amp_samp/2;
			fresamp2=25*25;                  /* Variance of guassian shaped filter */
			for ( i = -1; ++i <= c_n/2; )
			{
				fval = i*fres;
		...
		/* Stuff Amplitude */
		for ( j = -1; ++j < *len + 2*twindow_len; )
		{
			i_val = (j-twindow_len)/tstep;
