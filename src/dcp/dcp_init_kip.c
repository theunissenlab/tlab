...
int twindow=200;
   ...
   while (argc)
	{
		...
		stim_fband(stim,&nband,&nlen,sil_window,&stim_env,&f_low,&f_high,&f_width);
#else
		/* Read the data from the file directly */
			
		/* Read the hdr file to get nband and nlen */
        
		  fid=fopen("894df349.hdr","r");
			fscanf(fid,"%d %d",nlen,nband);
		  fclose(fid);

		/* Read the (stimulus data) data file and stuff it in stim_env */

        fid=fopen("894df349.mat","r");
		for ( it = -1; ++it < nlen; )
		{
					stim_env[it] = fread(fid,"double",nband);
		}
			end
#endif
   ...
   fprintf(f_count,"%d %d %d %f %f %f %d %d", nf, nband, twindow, f_low,f_high,f_width, lin_flg,sil_window);
   fclose(f_count);
