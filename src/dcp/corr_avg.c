...
#define CROSS_ERR 0.075


	if ( argc != 3 )
	{
		printf ("Error: wrong number of arguments. Usage:\n");
		printf ("corr_avg  <sig_file> <test_file>\n");
	}
   ...
   pre_points = (psig->pre*psig->adfreq)/1000;
	...
	printf("Mean test= %g Std from mean = %g Std from std = %g\n", avg_test,
	std_test, avg_std_test);
   ...
   for ( j = pre_points+lat_points; ++j < stim_end;)
	{
		cross_val += (wave_sig[j]-avg_sig)*(wave_test[j]-avg_test);
		cross_err += (wave_sig[j]-avg_sig)*(wave_sig[j]-avg_sig)*wave_std[j]*wave_std[j];
      cross_norm += (wave_sig[j]-avg_sig)*(wave_sig[j]-avg_sig);
	}
	cross_err = sqrt(cross_err);
   cross_val /= cross_norm;
   cross_err /= cross_norm;

   cross_err = CROSS_ERR;
   if ( cross_val > cross_err ) printf("Yes:");
	else printf("No:");
	printf("Cross-corr=%g +- %g \n", cross_val, cross_err);
